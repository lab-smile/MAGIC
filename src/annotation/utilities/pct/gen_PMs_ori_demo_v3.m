...
% ----------------------------------------------------
% CHANGE HERE
% CTP Toolbox Filepath
CTPToolbox = 'D:\Desktop Files\Dropbox (UFL)\SMILE\MAGIC\tutorial\aif_vof_annotation\pct';

% Display PMA Colormaps Filepath
PMAToolbox = 'D:\Desktop Files\Dropbox (UFL)\SMILE\MAGIC\tutorial\aif_vof_annotation\Display_PMA_Colormaps';

% Subject folder
SubjFolder = 'D:\Desktop Files\Dropbox (UFL)\SMILE\MAGIC\tutorial\aif_vof_annotation\Subjects';

% Patient ID (Set it to the folder id the data was found in)
PatientID = '10029901_1959';
...
p_ctp = genpath(CTPToolbox);
addpath(p_ctp);
p_pma = genpath(PMAToolbox);
addpath(p_pma);
...
volume = niftiread('converted_CTA_Head+_mA_Boost_20131207032518_3.nii');

%ctp_vol = volume.CTP_vol_ori;
%ctp_vol = squeeze(CTP_vol_ori(:,:,:,slice));
ctp_vol = squeeze(volume(:,:,slice,:));
